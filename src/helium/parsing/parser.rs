...
use crate::helium::parsing::error::ParserError::{
    ConstantCollision, Named, UnexpectedEOF, UnknownDirective,
};
...
use crate::helium::parsing::ParserError::{
    FileError, FileNotFound, IncludeLexError, UnknownIdentifier,
};
...
impl<'a> Parser<'a> {
    ...
    pub fn parse(mut self, root: Option<&ProgramTree>) -> Result<ProgramTree, Vec<ParserError>> {
        ...
        if root.is_none() {
            self.current_segment
                .elements
                .push(ProgramElement::Instruction(instructions::Instruction::new(
                    Halt,
                )))
        }
        ...
        if check.is_err() {
            for e in check.err().unwrap() {
                self.errors.push(UnknownIdentifier { name: e })
            }
        }
    ...
    fn parse_of_type(&mut self, expected: TokenKind) -> Result<&Token, ParserError> {
        let next = self.tokens.next_if(|t| t.kind == expected);

        if next.is_none() && self.tokens.peek().is_some() {
            return Err(ParserError::MismatchedTypes {
                expected,
    ...
    fn parse_identifier(&mut self, tree: &mut ProgramTree, token: &Token) {
        ...
        self.current_segment
            .elements
            .push(ProgramElement::Identifier(key.clone()));
    }
    ...
    fn parse_integer(&mut self, token: &Token) {
        ...
        self.current_segment
            .elements
            .push(ProgramElement::Immediate(value));
    }
    fn parse_instruction(&mut self, tree: &mut ProgramTree, token: &Token) {
        ...
        while let Some(next) = self.tokens.peek() {
            ...
            if token.kind == Integer {
                ...
                ins.args.push(Argument::ImmediateIdentifier(ident))
            } else {
                // Register case
                ...
                match value {
                    ...
                    ValueKind::Integer(i) => ins.args.push(Argument::Register(i)),
                    ValueKind::Word(w) => {
                        if !tree.has_const(&w) {
        ...
        self.current_segment
            .elements
            .push(ProgramElement::Instruction(ins));
    }
    fn parse_register(&mut self, token: &Token) {
        ...
        self.errors.push(match reg_key {
            ValueKind::Instruction(i) => Named {
                error: format!("Unexpected Token: Register({:?})", i),
            },
            ValueKind::Integer(i) => Named {
                error: format!("Unexpected Token: Register({})", i),
            },
            ValueKind::Word(w) => Named {
                error: format!("Unexpected Token: Register({})", w),
            },
        })
    }
    fn parse_directive(&mut self, tree: &mut ProgramTree, token: &Token) {
        ...
        match directive_name.as_str() {
            ...
            _ => {
                self.errors.push(UnknownDirective {
                    name: directive_name,
                });
    ...
    fn consume_expression(&mut self) {
        for next in self.tokens.by_ref() {
            if next.kind == SemiColon || next.kind == Newline {
                break;
            }
        }
    }
    fn parse_skipto_directive(&mut self, tree: &mut ProgramTree) {
        ...
        if next.is_none() {
            ...
            let name = format!(
                "__auto_label_{}_segment_{}__",
                &self.file_name, &tree.auto_label_id
            );
    ...
    fn parse_include_directive(&mut self, tree: &mut ProgramTree) {
        ...
        if tree.has_include(&include_name) {
            return;
        }
    ...
    fn consume_whitespaces(&mut self) {
        while let Some(token) = self.tokens.peek() {
            if token.kind != Newline && token.kind != Comma && token.kind != SemiColon {
                break;
            }
            self.tokens.next();
        }
